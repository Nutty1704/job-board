...
class Config:
    ...
    def from_environment(cls, environment: Mapping[str, str]) -> "Config":
        ...
        return cls(
            ...
            model=environment.get("OPENAI_MATCHING_MODEL", "gpt-5.6-luna").strip(),
...
class Matcher:
    ...
    def _process_many(self, events: list[dict[str, Any]]) -> tuple[list[str | None], list[Exception | None]]:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=min(5, len(leased))) as executor:
...
def openai_assessment(api_key: str, model: str, profile: Profile, event: Mapping[str, Any]) -> dict[str, Any]:
    ...
    try:
        with urlopen(request, timeout=45) as response:  # nosec B310 - fixed OpenAI URL
